 ...
 * @return string
 */
function theme_rutatic_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_rutatic_set_bgsimg($theme);
 ...
 * @return string
 */
function theme_rutatic_set_bgsimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_rutatic_get_main_scss_content($theme) {
    global $CFG;

    $scss = file_get_contents($CFG->dirroot . '/theme/boost/scss/preset/default.scss');

    // RutaTIC scss.
    $rutaticvariables = file_get_contents($CFG->dirroot . '/theme/rutatic/scss/rutatic/_variables.scss');
    $rutatic = file_get_contents($CFG->dirroot . '/theme/rutatic/scss/rutatic.scss');

    // Combine them together.
    return $rutaticvariables . "\n" . $scss . "\n" . $rutatic;
}
 ...
 * @return string
 */
function theme_rutatic_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_rutatic_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('rutatic');
 ...
 * @return string
 */
function theme_rutatic_get_setting($setting, $format = false) {
    $theme = theme_config::load('rutatic');
 ...
 */
function theme_rutatic_boostnavigation_extend_navigation(global_navigation $navigation) {
    global $CFG;
    ...
    if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
        ...
        foreach ($mycourseschildrennodeskeys as $k) {
            ...
            if ($CFG->navshowmycoursecategories) {
                ...
                $allchildrennodes = theme_rutatic_boostnavigation_get_all_childrenkeys($mycoursesnode->get($k));
 ...
 * @return array
 */
function theme_rutatic_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    foreach ($childrennodeskeys as $ck) {
        $allchildren = array_merge($allchildren, theme_rutatic_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
    }
